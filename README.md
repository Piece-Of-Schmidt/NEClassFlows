...
* **📈 Flow Analysis**: aggregate entity mentions over time (e.g., monthly shares of "USA" or "China" coverage) at both entity and document levels.
* **🏆 Top-K Extraction**: Automatically identify the most salient actors or countries per period (e.g., "Who were the top 5 foreign actors in 2015?").
* **🧹 Data Harmonization**: flexible tools to merge synonyms and resolve conflicting labels based on custom dictionaries.
* **📊 Document-Level Aggregation**: Convert entity-level data into document-feature matrices (wide format) for regression analysis or structural topic modeling.
